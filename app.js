...
function productCard(p){
  return `<a class="product-card" href="${productUrl(p)}">
    ...
    <div class="product-card-body">
      ...
      <span class="tag">Copyright Record</span>
    </div>
  </a>`;
}
  ...
  root.innerHTML = `
    ...
    <div class="product-layout">
      ...
      <div class="product-info">
        ...
        <div class="rating">★★★★★ <span style="color:#777">Original product image record</span></div>
        <div class="info-line"><b>Brands</b><span>${escapeHtml(p.brand || SITE_NAME)}</span></div>
        <div class="info-line"><b>SKU</b><span>${escapeHtml(p.sku)}</span></div>
        <div class="info-line"><b>ASIN</b><span>${escapeHtml(p.asin)}</span></div>
        <div class="info-line"><b>First Release Time</b><span>${escapeHtml(p.first_release_time)}</span></div>
    ...
    <div class="tabs">
      <div class="tab-head"><div>Description</div><div>Copyright Notice</div></div>
      <div class="tab-body">
        <p>${escapeHtml(p.description)}</p>
        <p><b>Rights Owner:</b> ${RIGHTS_OWNER} &nbsp; | &nbsp; <b>Contact:</b> ${RIGHTS_EMAIL}</p>
        <p>Unauthorized copying, reproduction, redistribution, listing upload, or commercial use of product images and listing content is prohibited.</p>
      </div>
    </div>`;
}
